function varargout = FTransformada(varargin)
% FTRANSFORMADA MATLAB code for FTransformada.fig
%      FTRANSFORMADA, by itself, creates a new FTRANSFORMADA or raises the existing
%      singleton*.
%
%      H = FTRANSFORMADA returns the handle to a new FTRANSFORMADA or the handle to
%      the existing singleton*.
%
%      FTRANSFORMADA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FTRANSFORMADA.M with the given input arguments.
%
%      FTRANSFORMADA('Property','Value',...) creates a new FTRANSFORMADA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FTransformada_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FTransformada_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FTransformada

% Last Modified by GUIDE v2.5 25-May-2017 06:13:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FTransformada_OpeningFcn, ...
                   'gui_OutputFcn',  @FTransformada_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FTransformada is made visible.
function FTransformada_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FTransformada (see VARARGIN)

% Choose default command line output for FTransformada
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes FTransformada wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FTransformada_OutputFcn(hObject, eventdata, handles) 
...
syms t;
%
c0 = (1/T)*(int(xt1,t,get(handles.LimInf1,'string'),str2num(get(handles.LimSup1,'string'))) + int(xt2,t,get(handles.LimInf2,'string'),str2num(get(handles.LimSup2,'string'))) + int(xt3,t,get(handles.LimInf3,'string'),str2num(get(handles.LimSup3,'string'))));
Cn = (1/T)*(int(xt1.*exp(-j*n*w0*t),t,get(handles.LimInf1,'string'),str2num(get(handles.LimSup1,'string'))) + int(xt2.*exp(-j*n*w0*t),t,get(handles.LimInf2,'string'),str2num(get(handles.LimSup2,'string'))) + int(xt3.*exp(-j*n*w0*t),t,get(handles.LimInf3,'string'),str2num(get(handles.LimSup3,'string'))));
...
plot(handles.Dominio_Frecuencia,w,real(ans),'g','linewidth',2);
title(' Transformada de Fourier ');
xlabel(' Dominio de w ');
ylabel(' F(w) ');
grid on;

plot(handles.Espectro_Amplitud,w,imag(ans),'r','linewidth',2);
